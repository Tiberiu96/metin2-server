...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot trade in the shop while another window is open.", pkChr->GetLanguage()));
		return false;
	}
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You are too far away from the shop to buy something.", ch->GetLanguage()));
		return;
	}
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, BYTE bCount)
{
	...
	if (DISTANCE_APPROX(ch->GetX()-ch->GetShopOwner()->GetX(), ch->GetY()-ch->GetShopOwner()->GetY())>2000)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You are too far away from the shop to sell something.", ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Equipped items cannot be sold.", ch->GetLanguage()));
		return;
	}
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot trade because you are carrying more than 2 billion Yang.", ch->GetLanguage()));
		return;
	}
	...
	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This sale will be taxed %d%%.", ch->GetLanguage()), iVal);
