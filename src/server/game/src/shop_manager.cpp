...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("다른 거래창이 열린상태에서는 상점거래를 할수 가 없습니다."), pkChr->GetLanguage()));
		return false;
	}
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("상점과의 거리가 너무 멀어 물건을 살 수 없습니다."), ch->GetLanguage()));
		return;
	}
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, BYTE bCount)
{
	...
	if (DISTANCE_APPROX(ch->GetX()-ch->GetShopOwner()->GetX(), ch->GetY()-ch->GetShopOwner()->GetY())>2000)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("상점과의 거리가 너무 멀어 물건을 팔 수 없습니다."), ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("착용 중인 아이템은 판매할 수 없습니다."), ch->GetLanguage()));
		return;
	}
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("20억냥이 초과하여 물품을 팔수 없습니다."), ch->GetLanguage()));
		return;
	}
	...
	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("판매금액의 %d %% 가 세금으로 나가게됩니다"), ch->GetLanguage()), iVal);
