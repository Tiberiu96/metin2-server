...
void Cube_open (LPCHARACTER ch)
{
	...
	if (ch->IsCubeOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("이미 제조창이 열려있습니다."), ch->GetLanguage()));
		return;
	}
	if ( ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen() )
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("다른 거래중(창고,교환,상점)에는 사용할 수 없습니다."), ch->GetLanguage()));
		return;
	}
...
bool Cube_make (LPCHARACTER ch)
{
	...
	if (!(ch)->IsCubeOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("제조창이 열려있지 않습니다"), ch->GetLanguage()));
		return false;
	}
	...
	if (NULL == cube_proto)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("제조 재료가 부족합니다"), ch->GetLanguage()));
		return false;
	}

	if (ch->GetGold() < cube_proto->gold)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("돈이 부족하거나 아이템이 제자리에 없습니다."), ch->GetLanguage()));	// 이 텍스트는 이미 널리 쓰이는거라 추가번역 필요 없음
		return false;
	}
	...
	{
		// 실패
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("제조에 실패하였습니다."), ch->GetLanguage()));	// 2012.11.12 새로 추가된 메세지 (locale_string.txt 에 추가해야 함)
