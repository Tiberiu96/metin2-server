...
void Cube_open (LPCHARACTER ch)
{
	...
	if (ch->IsCubeOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The Build window is already open.", ch->GetLanguage()));
		return;
	}
	if ( ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen() )
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot build something while another trade/storeroom window is open.", ch->GetLanguage()));
		return;
	}
...
bool Cube_make (LPCHARACTER ch)
{
	...
	if (!(ch)->IsCubeOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The build window is not open.", ch->GetLanguage()));
		return false;
	}
	...
	if (NULL == cube_proto)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You do not have the right material.", ch->GetLanguage()));
		return false;
	}

	if (ch->GetGold() < cube_proto->gold)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Not enough Yang or not enough space in the inventory.", ch->GetLanguage()));	// 이 텍스트는 이미 널리 쓰이는거라 추가번역 필요 없음
		return false;
	}
	...
	{
		// 실패
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Crafting failed.", ch->GetLanguage()));	// 2012.11.12 새로 추가된 메세지 (locale_string.txt 에 추가해야 함)
