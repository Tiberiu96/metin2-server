...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You can participate in the guild battle in viewer mode.", ch->GetLanguage()));
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("While choosing a character, an icon appears.", ch->GetLanguage()));
	}
