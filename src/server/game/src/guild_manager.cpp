...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Guild] This guild name is invalid.", gcp.master->GetLanguage()));
		return 0;
	}
	...
	if (pmsg->Get()->uiNumRows > 0)
	{
		...
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Guild] This guild name is already taken.", gcp.master->GetLanguage()));
			return 0;
		}
	}
	else
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Guild] You cannot found a guild.", gcp.master->GetLanguage()));
		return 0;
	}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Guild] %s has refused the Guild war.", g2->GetMasterCharacter()->GetLanguage()), g1->GetName());
	}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Guild] The guild war has been cancelled.", master1->GetLanguage()));
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Guild] The guild war has been cancelled.", master2->GetLanguage()));
	}
