...
ACMD(do_user_horse_ride)
{
	...
	if (ch->IsHorseRiding() == false)
	{
		...
		if (ch->GetMountVnum())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�̹� Ż���� �̿����Դϴ�."), ch->GetLanguage()));
			return;
		}

		if (ch->GetHorse() == NULL)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���� ���� ��ȯ���ּ���."), ch->GetLanguage()));
			return;
		}
...
ACMD(do_user_horse_back)
{
	if (ch->GetHorse() != NULL)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���� �������½��ϴ�."), ch->GetLanguage()));
	}
	else if (ch->IsHorseRiding() == true)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("������ ���� ������ �մϴ�."), ch->GetLanguage()));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���� ���� ��ȯ���ּ���."), ch->GetLanguage()));
...
ACMD(do_user_horse_feed)
{
	...
	if (ch->GetHorse() == NULL)
	{
		if (ch->IsHorseRiding() == false)
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���� ź ���¿����� ���̸� �� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}
	...
	if (ch->CountSpecifyItem(dwFood) > 0)
	{
		...
		ch->FeedHorse();
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("������ %s%s �־����ϴ�."), ch->GetLanguage()), 
				ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName,
				g_iUseLocale ? "" : under_han(ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName) ? LC_TEXT("��") : LC_TEXT("��"));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s �������� �ʿ��մϴ�"), ch->GetLanguage()), ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName);
	}
}
...
EVENTFUNC(timed_event)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%d�� ���ҽ��ϴ�."), ch->GetLanguage()), info->left_second);
		--info->left_second;
	}
...
ACMD(do_cmd)
{
	/* RECALL_DELAY
	   ...
	   {
	   ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("��� �Ǿ����ϴ�."), ch->GetLanguage()));
	...
	{
		case SCMD_LOGOUT:
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�α��� ȭ������ ���� ���ϴ�. ��ø� ��ٸ�����."), ch->GetLanguage()));
			break;

		case SCMD_QUIT:
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("������ ���� �մϴ�. ��ø� ��ٸ�����."), ch->GetLanguage()));
			break;

		case SCMD_PHASE_SELECT:
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("ĳ���͸� ��ȯ �մϴ�. ��ø� ��ٸ�����."), ch->GetLanguage()));
			break;
	}
...
ACMD(do_mount)
{
	...
	if (distance > 600.0f)
	{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�� �� ������ ���� ��������."), ch->GetLanguage()));
	return;
	}
	...
	{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�ű⿡�� Ż �� �����."), ch->GetLanguage()));
	return;
	}
	...
	if (distance > 600.0f)
	{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�� �� ������ ���� Ÿ����."), ch->GetLanguage()));
	return;
	}
...
ACMD(do_restart)
{
	...
	if (subcmd != SCMD_RESTART_TOWN && (!ch->GetWarMap() || ch->GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG))
	{
		if (!test_server)
		{
			if (ch->IsHack())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���� ����� �� �� �����ϴ�. (%d�� ����)"), ch->GetLanguage()), iTimeToDead - (180 - g_nPortalLimitTime));
					return;
				}
			}
			...
			if (iTimeToDead > 170)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���� ����� �� �� �����ϴ�. (%d�� ����)"), ch->GetLanguage()), iTimeToDead - 170);
				return;
			}
		}
	}
	...
	if (subcmd == SCMD_RESTART_TOWN)
	{
		...
		if (iTimeToDead > 173)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���� �������� ����� �� �� �����ϴ�. (%d �� ����)"), ch->GetLanguage()), iTimeToDead - 173);
			return;
		}
	}
	//END_PREVENT_HACK
	...
	if (1 == quest::CQuestManager::instance().GetEventFlag("threeway_war"))
	{
		if (subcmd == SCMD_RESTART_TOWN || subcmd == SCMD_RESTART_HERE)
		{
			...
			if (true == CThreeWayWar::instance().IsSungZiMapIndex(ch->GetMapIndex()))
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�������� ��Ȱ ��ȸ�� ��� �Ҿ����ϴ�! ������ �̵��մϴ�!"), ch->GetLanguage()));
					ch->WarpSet(EMPIRE_START_X(ch->GetEmpire()), EMPIRE_START_Y(ch->GetEmpire()));
				}
				else
				{
...
ACMD(do_stat_minus)
{
	...
	if (ch->IsPolymorphed())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�а� �߿��� �ɷ��� �ø� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}
...
ACMD(do_pvp)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("����忡�� ����Ͻ� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}
	...
	{
		pkVictim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("������ ������Դϴ�."), pkVictim->GetLanguage()));
		return;
	}
...
ACMD(do_guildskillup)
{
	...
	if (!ch->GetGuild())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> ��忡 �������� �ʽ��ϴ�."), ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> ��� ��ų ������ ������ ������ �����ϴ�."), ch->GetLanguage()));
	}
}
...
ACMD(do_safebox_change_password)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<â��> �߸��� ��ȣ�� �Է��ϼ̽��ϴ�."), ch->GetLanguage()));
		return;
	}
	...
	if (LC_IsBrazil() == true)
	{
		for (int i = 0; i < 6; ++i)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<â��> ��й�ȣ�� �����ڸ� �����մϴ�."), ch->GetLanguage()));
				return;
			}
		}
	}
...
ACMD(do_mall_password)
{
	...
	if (ch->GetMall())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<â��> â���� �̹� �����ֽ��ϴ�."), ch->GetLanguage()));
		return;
	}

	if (iPulse - ch->GetMallLoadTime() < passes_per_sec * 10) // 10�ʿ� �ѹ��� ��û ����
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<â��> â���� ������ 10�� �ȿ��� �� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}
...
ACMD(do_ungroup)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<��Ƽ> ���� ������ ��Ƽ ���� ó���� �� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}

	if (ch->GetDungeon())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<��Ƽ> ���� �ȿ����� ��Ƽ���� ���� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<��Ƽ> ��Ƽ���� �����̽��ϴ�."), ch->GetLanguage()));
...
ACMD(do_war)
{
	...
	if (g->UnderAnyWar())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> �̹� �ٸ� ���￡ ���� �� �Դϴ�."), ch->GetLanguage()));
		return;
	}
	...
	if (!opp_g)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> �׷� ��尡 �����ϴ�."), ch->GetLanguage()));
		return;
	}
	...
	switch (g->GetGuildWarState(opp_g->GetID()))
	{
		case GUILD_WAR_NONE:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> ���� ��尡 �̹� ���� �� �Դϴ�."), ch->GetLanguage()));
					return;
				}
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> ���� �����Ͽ� ������� �� �� �����ϴ�."), ch->GetLanguage()));
					return;
				}
		...
		case GUILD_WAR_SEND_DECLARE:
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�̹� �������� ���� ����Դϴ�."), ch->GetLanguage()));
				return;
			}
			break;
	...
	if (!g->CanStartWar(type))
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> ���� ������ ���ڶ� ������� �� �� �����ϴ�."), ch->GetLanguage()));
			sys_log(0, "GuildWar.StartError.NEED_LADDER_POINT");
		}
		else if (g->GetMemberCount() < GUILD_WAR_MIN_MEMBER_COUNT)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> ������� �ϱ� ���ؼ� �ּ��� %d���� �־�� �մϴ�."), ch->GetLanguage()), GUILD_WAR_MIN_MEMBER_COUNT);
	...
	do
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<���> ���� ����� ������� �������� �ƴմϴ�."), ch->GetLanguage()));
		g->RequestRefuseWar(opp_g->GetID());
		return;
...
ACMD(do_messenger_auth)
{
	...
	if (answer != 'y')
	{
		...
		if (tch)
			tch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s ������ ���� ģ�� ����� �ź� ���߽��ϴ�."), tch->GetLanguage()), ch->GetName());
	}
...
ACMD(do_unmount)
{
	...
	{
		ch->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�κ��丮�� �� ���� ���� �� �����ϴ�."), ch->GetLanguage()));
	}
...
ACMD(do_view_equip)
{
	...
	if (*arg1)
	{
		...
		/*
		   ...
		   if (ch->GetSP() < iSPCost)
		   {
		   ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���ŷ��� �����Ͽ� �ٸ� ����� ��� �� �� �����ϴ�."), ch->GetLanguage()));
		   return;
		   }
...
ACMD(do_party_request)
{
	...
	if (ch->GetParty())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�̹� ��Ƽ�� ���� �����Ƿ� ���Խ�û�� �� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}
...
ACMD(do_monarch_warpto)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���ָ��� ��� ������ ����Դϴ�"), ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%d �ʰ� ��Ÿ���� �������Դϴ�."), ch->GetLanguage()), ch->GetMCLTime(CHARACTER::MI_WARP));
		return;
	}
	...
	if (!CMonarch::instance().IsMoneyOk(WarpPrice, ch->GetEmpire()))
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("������ ���� �����մϴ�. ���� : %u �ʿ�ݾ� : %u"), ch->GetLanguage()), NationMoney, WarpPrice);
		return;	
	}
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("����: warpto <character name>"), ch->GetLanguage()));
		return;
	}
	...
	if (!tch)
	{
		...
		if (pkCCI)
		{
			...
			{
				ch->ChatPacket (CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("Ÿ���� �������Դ� �̵��Ҽ� �����ϴ�"), ch->GetLanguage()));
				return;
			}

			if (pkCCI->bChannel != g_bChannel)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�ش� ������ %d ä�ο� �ֽ��ϴ�. (���� ä�� %d)"), ch->GetLanguage()), pkCCI->bChannel, g_bChannel);
				return;
			}
			if (!IsMonarchWarpZone(pkCCI->lMapIndex))
			{
				ch->ChatPacket (CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�ش� �������� �̵��� �� �����ϴ�."), ch->GetLanguage()));
				return;
			}
			...
			else
			{
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s ���Է� �̵��մϴ�"), ch->GetLanguage()), arg1);
				ch->WarpSet(pos.x, pos.y);
...
ACMD(do_monarch_transfer)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("����: transfer <name>"), ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%d �ʰ� ��Ÿ���� �������Դϴ�."), ch->GetLanguage()), ch->GetMCLTime(CHARACTER::MI_TRANSFER));	
		return;
	}
	...
	if (!tch)
	{
		...
		if (pkCCI)
		{
			if (pkCCI->bEmpire != ch->GetEmpire())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�ٸ� ���� ������ ��ȯ�� �� �����ϴ�."), ch->GetLanguage()));
				return;
			}
			if (pkCCI->bChannel != g_bChannel)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s ���� %d ä�ο� ���� �� �Դϴ�. (���� ä��: %d)"), ch->GetLanguage()), arg1, pkCCI->bChannel, g_bChannel);
				return;
			}
			if (!IsMonarchWarpZone(pkCCI->lMapIndex))
			{
			...
			{
				ch->ChatPacket (CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�ش� �������� ��ȯ�� �� �����ϴ�."), ch->GetLanguage()));
				return;
			}
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s ���� ��ȯ�Ͽ����ϴ�."), ch->GetLanguage()), arg1);
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�Է��Ͻ� �̸��� ���� ����ڰ� �����ϴ�."), ch->GetLanguage()));
		}
	...
	if (ch == tch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�ڽ��� ��ȯ�� �� �����ϴ�."), ch->GetLanguage()));
		return;
	}
...
ACMD(do_monarch_info)
{
	if (CMonarch::instance().IsMonarch(ch->GetPlayerID(), ch->GetEmpire()))	
	{
		...
		{
			if (n == ch->GetEmpire())
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("[%s����] : %s  �����ݾ� %lld "), ch->GetLanguage()), EMPIRE_NAME(n), p->name[n], p->money[n]);
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("[%s����] : %s  "), ch->GetLanguage()), EMPIRE_NAME(n), p->name[n]);
...
ACMD(do_monarch_tax)
{
	...
	if (!ch->IsMonarch())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("���ָ��� ����Ҽ� �ִ� ����Դϴ�"), ch->GetLanguage()));
		return;
	}
	...
	if (tax < 1 || tax > 50)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("1-50 ������ ��ġ�� �������ּ���"), ch->GetLanguage()));

	quest::CQuestManager::instance().SetEventFlag("trade_tax", tax); 

	// ���ֿ��� �޼��� �ϳ�
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("������ %d %�� �����Ǿ����ϴ�"), ch->GetLanguage()));
...
ACMD(do_monarch_mob)
{
	...
	if (LC_IsYMIR() == true || LC_IsKorea() == true)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("�ڱ� ���信���� ����� �� �ִ� ����Դϴ�"), ch->GetLanguage()));
			return;
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%d �ʰ� ��Ÿ���� �������Դϴ�."), ch->GetLanguage()), ch->GetMCLTime(CHARACTER::MI_SUMMON));	
		return;
	}
	...
	{
		int NationMoney = CMonarch::instance().GetMoney(ch->GetEmpire());
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("������ ���� �����մϴ�. ���� : %u �ʿ�ݾ� : %u"), ch->GetLanguage()), NationMoney, SummonPrice);
		return;	
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("��ȯ�Ҽ� ���� ���� �Դϴ�. ��ȯ������ ���ʹ� Ȩ�������� �����ϼ���"), ch->GetLanguage()));
		return;
	}
...
ACMD(do_in_game_mall)
{
...

	{
		...
		// inner = MD5(pid + aid + ts + KEY1)
		snprintf(buf, sizeof(buf), "%u%u%ld%s",
			ch->GetPlayerID(), ch->GetAID(), ts, g_strMallKey1.c_str());
		MD5Init(&ctx);
		MD5Update(&ctx, (const unsigned char*)buf, strlen(buf));
		MD5Final(digest, &ctx);
		for (i = 0; i < 16; ++i) {
			inner_hex[i+i]   = hex[digest[i] >> 4];
			inner_hex[i+i+1] = hex[digest[i] & 0x0f];
		}
		inner_hex[32] = '\0';

		// sas = MD5(inner_hex + KEY2)
		snprintf(buf, sizeof(buf), "%s%s", inner_hex, g_strMallKey2.c_str());
		MD5Init(&ctx);
		MD5Update(&ctx, (const unsigned char*)buf, strlen(buf));
		MD5Final(digest, &ctx);
		for (i = 0; i < 16; ++i) {
		...
		snprintf(buf, sizeof(buf), "mall http://%s/ishop?pid=%u&c=%s&sid=%d&ts=%ld&sas=%s",
			g_strWebMallURL.c_str(), ch->GetPlayerID(), country_code, g_server_id, ts, sas);
...
ACMD(do_dice) 
{
	...
	if (ch->GetParty())
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("����� �ֻ����� ���� %d�� ���Խ��ϴ�. (%d-%d)"), ch->GetLanguage()), n, start, end);
}
