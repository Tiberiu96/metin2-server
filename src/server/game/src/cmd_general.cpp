...
ACMD(do_in_game_mall)
{
	...
	if (LC_IsEurope() == true)
	{
		...
		char sas[33];
		MD5_CTX ctx;
		const char sas_key1[] = "xK9#mQ4vL2$pR7nW";
		const char sas_key2[] = "jT8&bZ3cF6!hY5sA";
		time_t now = time(0);

		// First hash: md5(pid + account_id + timestamp + key1)
		snprintf(buf, sizeof(buf), "%u%u%ld%s", ch->GetPlayerID(), ch->GetAID(), (long)now, sas_key1);
		...
		int i;
		for (i = 0; i < 16; ++i) {
			hash1[i+i] = hex[digest[i] >> 4];
			hash1[i+i+1] = hex[digest[i] & 0x0f];
		}
		hash1[i+i] = '\0';
#endif

		// Second hash: md5(hash1 + key2)
		snprintf(buf, sizeof(buf), "%s%s", hash1, sas_key2);
		...
		snprintf(buf, sizeof(buf), "mall http://%s/ishop?pid=%u&c=%s&sid=%d&sas=%s&ts=%ld",
				g_strWebMallURL.c_str(), ch->GetPlayerID(), country_code, g_server_id, sas, (long)now);
