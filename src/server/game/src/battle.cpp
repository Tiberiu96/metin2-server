...
int battle_melee_attack(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	if (timed_event_cancel(ch))
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Action cancelled. You have entered a battle.", ch->GetLanguage()));

	if (timed_event_cancel(victim))
		victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Action cancelled. You have entered a battle.", victim->GetLanguage()));
