...
bool CPolymorphUtils::PolymorphCharacter(LPCHARACTER pChar, LPITEM pItem, const CMob* pMob)
{
	...
	if (iPolyPercent <= 0)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Transformation error.", pChar->GetLanguage()));
		return false;
	}
	else
	{
...
bool CPolymorphUtils::UpdateBookPracticeGrade(LPCHARACTER pChar, LPITEM pItem)
{
	...
	if (pItem->GetSocket(1) > 0)
		...
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You learned the transmutation completely.", pChar->GetLanguage()));
