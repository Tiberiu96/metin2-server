...
void MessengerManager::RequestToAdd(LPCHARACTER ch, LPCHARACTER target)
{
	...
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The other player cannot receive friend requests.", ch->GetLanguage()));
	    return;
	}
...
void MessengerManager::__AddToList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (d)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Friends] You have added %s as a friend.", ch->GetLanguage()), companion.c_str());
	}
...
void MessengerManager::__RemoveFromList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (d)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Friends] You have deleted %s from the list.", ch->GetLanguage()), companion.c_str());
}
