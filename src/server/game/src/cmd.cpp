...
ACMD(do_inputall)
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("명령어를 모두 입력하세요."), ch->GetLanguage()));
}
...
void interpret_command(LPCHARACTER ch, const char * argument, size_t len)
{
	...
	if (ch->GetPosition() < cmd_info[icmd].minimum_position)
	{
		...
		{
			case POS_MOUNTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("탄 상태에서는 할 수 없습니다."), ch->GetLanguage()));
				break;

			case POS_DEAD:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("쓰러진 상태에서는 할 수 없습니다."), ch->GetLanguage()));
				break;

			case POS_SLEEPING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("꿈속에서 어떻게요?"), ch->GetLanguage()));
				break;

			case POS_RESTING:
			case POS_SITTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("먼저 일어 나세요."), ch->GetLanguage()));
				break;
				/*
				   case POS_FIGHTING:
				   ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("목숨을 걸고 전투 중 입니다. 집중 하세요."), ch->GetLanguage()));
				   break;
				 */
			default:
