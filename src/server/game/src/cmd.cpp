...
ACMD(do_inputall)
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Please enter the Order in full length.", ch->GetLanguage()));
}
...
void interpret_command(LPCHARACTER ch, const char * argument, size_t len)
{
	...
	if (ch->GetPosition() < cmd_info[icmd].minimum_position)
	{
		...
		{
			case POS_MOUNTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot do this whilst sitting on a Horse.", ch->GetLanguage()));
				break;

			case POS_DEAD:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot do that while you are lying on the ground.", ch->GetLanguage()));
				break;

			case POS_SLEEPING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("In my Dreams? What?", ch->GetLanguage()));
				break;

			case POS_RESTING:
			case POS_SITTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Get up first.", ch->GetLanguage()));
				break;
				/*
				   case POS_FIGHTING:
				   ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You are in a life-or-death battle. Stay focused.", ch->GetLanguage()));
				   break;
				 */
			default:
