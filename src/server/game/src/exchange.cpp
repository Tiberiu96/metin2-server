...
bool CHARACTER::ExchangeStart(LPCHARACTER victim)
{
	...
	if (IsObserverMode())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot trade while observing.", GetLanguage()));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The other person has cancelled the trade.", GetLanguage()));
		return false;
	}
...
bool CExchange::AddItem(TItemPos item_pos, BYTE display_pos)
{
	...
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot trade this item.", m_pOwner->GetLanguage()));
		return false;
	}
...
bool CExchange::CheckSpace()
{
	...
	for (i = 0; i < EXCHANGE_ITEM_MAX_NUM; ++i)
	{
		...
		else
		{
			...
			if (iPos >= 0) { s_grid1.Put(iPos, 1, item->GetSize()); continue; }

			iPos = s_grid2.FindBlank(1, item->GetSize());
			if (iPos >= 0) { s_grid2.Put(iPos, 1, item->GetSize()); continue; }

			iPos = s_grid3.FindBlank(1, item->GetSize());
			if (iPos >= 0) { s_grid3.Put(iPos, 1, item->GetSize()); continue; }

			iPos = s_grid4.FindBlank(1, item->GetSize());
			if (iPos >= 0) { s_grid4.Put(iPos, 1, item->GetSize()); continue; }
...
bool CExchange::Accept(bool bAccept)
{
	...
	if (m_bAccept && GetCompany()->m_bAccept)
	{
		...
		if (!Check(&iItemCount))
		{
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Not enough Yang or not enough space in the inventory.", GetOwner()->GetLanguage()));
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The other person does not have enough Yang or does not have any space left in their inventory.", victim->GetLanguage()));
			goto EXCHANGE_END;
		}
		...
		if (!CheckSpace())
		{
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The other person has no space left in their inventory.", GetOwner()->GetLanguage()));
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("There isn't enough space in your inventory.", victim->GetLanguage()));
			goto EXCHANGE_END;
		}
		...
		{
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Not enough Yang or not enough space in the inventory.", victim->GetLanguage()));
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The other person does not have enough Yang or does not have any space left in their inventory.", GetOwner()->GetLanguage()));
			goto EXCHANGE_END;
		}

		if (!GetCompany()->CheckSpace())
		{
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The other person has no space left in their inventory.", victim->GetLanguage()));
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("There isn't enough space in your inventory.", GetOwner()->GetLanguage()));
			goto EXCHANGE_END;
		}
		...
		if (Done())
		{
			...
			if (GetCompany()->Done())
			{
				...
				GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The trade with %s has been successful.", GetOwner()->GetLanguage()), victim->GetName());
				victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The trade with %s has been successful.", victim->GetLanguage()), GetOwner()->GetName());
