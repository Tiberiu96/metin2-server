...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("운영자 제제로 채팅이 금지 되었습니다."), GetLanguage()));
	}
	// END_OF_CHAT_BLOCK
...
bool CHARACTER::RemoveAffect(DWORD dwType)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("채팅 금지가 풀렸습니다."), GetLanguage()));
	}
	// END_OF_CHAT_BLOCK
