...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("(%s) guild has been created. [Temporary]", ch->GetLanguage()), cp.name);
}
...
ACMD(do_gwlist)
{
	ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_LANG("This guild is at war.", ch->GetLanguage()));
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("%s: This guild does not exist.", ch->GetLanguage()), arg1);
	}
}
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("A guild with this name or number does not exist.", ch->GetLanguage()));
		else
		{
			char buf[1024+1];
...
ACMD(do_block_chat_list)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This command does not exist.", ch->GetLanguage()));
		return;
	}
		...
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The building does not exist.", ch->GetLanguage()));
					return;
				}
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This type of building can only be erected once.", ch->GetLanguage()));
						return;
					}
				}
				...
				if (t->dwDependOnGroupVnum)
				{
					...
					//		if (dependent)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The Main Building has to be erected first.", ch->GetLanguage()));
							return;
						}
					}
				}
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Building failed because of incorrect pricing.", ch->GetLanguage()));
						return;
					}

					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your guild does not have enough Yang to erect this building.", ch->GetLanguage()));
						return;
					}
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You do not have enough resources to build a building.", ch->GetLanguage()));
							return;
						}
					}
				}
				...
				{
					if (test_server)
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot erect a building at this place.", ch->GetLanguage()));
					return;
				}
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This character does not exist.", ch->GetLanguage()));
		return;
	}
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Duel has not been successfully cancelled.", ch->GetLanguage()));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Duel cancelled successfully.", ch->GetLanguage()));
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Group] You have left the group.", pChar1->GetLanguage()));
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Group] You have left the group.", pChar2->GetLanguage()));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The duel has been successfully started.", ch->GetLanguage()));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("There is a problem with initiating the duel.", ch->GetLanguage()));
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("There are no combatants.", ch->GetLanguage()));
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("No status points left.", ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Remaining status points are too low.", ch->GetLanguage()));
		return;
	}

	if (nPoint < 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You entered an incorrect value.", ch->GetLanguage()));
		return;
	}
	...
	switch (subcmd)
	{
		...
		default :
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Suborder or the Order is incorrect.", ch->GetLanguage()));
			return;
			break;
	}
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Broken contract between player %d and player %d.", ch->GetLanguage()), pids.pid1, pids.pid2);
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Information for the Kingdoms", ch->GetLanguage()));
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Choose the Map Information of the Holy Land %d Entrance %d %d %d", ch->GetLanguage()), GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3)); 
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Information about the status of the kingdom battle", ch->GetLanguage()));
...
ACMD(do_set_stat)
{
	...
	else
	{
		...
		case POINT_HT:
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("Cannot set stat under initial stat."), ch->GetLanguage()));
				return;
			}
			n = 0;
			break;
