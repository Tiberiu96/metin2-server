...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("(%s) 길드가 생성되었습니다. [임시]"), ch->GetLanguage()), cp.name);
}
...
ACMD(do_gwlist)
{
	ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_LANG(LC_TEXT("현재 전쟁중인 길드 입니다"), ch->GetLanguage()));
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s: 존재하지 않는 길드 입니다."), ch->GetLanguage()), arg1);
	}
}
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("그런 이름 또는 번호의 길드가 없습니다."), ch->GetLanguage()));
		else
		{
			char buf[1024+1];
...
ACMD(do_block_chat_list)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("그런 명령어는 없습니다"), ch->GetLanguage()));
		return;
	}
		...
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("존재하지 않는 건물입니다."), ch->GetLanguage()));
					return;
				}
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("같이 지을 수 없는 종류의 건물이 지어져 있습니다."), ch->GetLanguage()));
						return;
					}
				}
				...
				if (t->dwDependOnGroupVnum)
				{
					...
					//		if (dependent)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("건설에 필요한 건물이 지어져 있지 않습니다."), ch->GetLanguage()));
							return;
						}
					}
				}
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("건물 비용 정보 이상으로 건설 작업에 실패했습니다."), ch->GetLanguage()));
						return;
					}

					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("건설 비용이 부족합니다."), ch->GetLanguage()));
						return;
					}
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("자재가 부족하여 건설할 수 없습니다."), ch->GetLanguage()));
							return;
						}
					}
				}
				...
				{
					if (test_server)
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("건물을 지을 수 없는 위치입니다."), ch->GetLanguage()));
					return;
				}
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("존재하지 않는 캐릭터 입니다."), ch->GetLanguage()));
		return;
	}
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("대련 강제 종료 실패"), ch->GetLanguage()));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("대련 강제 종료 성공"), ch->GetLanguage()));
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<파티> 파티에서 나가셨습니다."), pChar1->GetLanguage()));
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<파티> 파티에서 나가셨습니다."), pChar2->GetLanguage()));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("대련이 성공적으로 시작 되었습니다."), ch->GetLanguage()));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("대련 시작에 문제가 있습니다."), ch->GetLanguage()));
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("대련자가 없습니다."), ch->GetLanguage()));
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("남은 스탯 포인트가 없습니다."), ch->GetLanguage()));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("남은 스탯 포인트가 적습니다."), ch->GetLanguage()));
		return;
	}

	if (nPoint < 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("값을 잘못 입력하였습니다."), ch->GetLanguage()));
		return;
	}
	...
	switch (subcmd)
	{
		...
		default :
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("명령어의 서브 커맨드가 잘못 되었습니다."), ch->GetLanguage()));
			return;
			break;
	}
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("플레이어 %d 와 플레이어  %d를 파혼시킵니다.."), ch->GetLanguage()), pids.pid1, pids.pid2);
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("각제국 진행 정보"), ch->GetLanguage()));
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("선택 맵 정보 성지 %d 통로 %d %d %d"), ch->GetLanguage()), GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3)); 
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("나의 삼거리 진행정보"), ch->GetLanguage()));
