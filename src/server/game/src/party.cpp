	...
	while (it != m_memberMap.end())
	{
		...
		if (rMember.pCharacter)
		{
			if (rMember.pCharacter->GetDesc())
			{
				...
				rMember.pCharacter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Group] This group has been disbanded.", rMember.pCharacter->GetLanguage()));
			}
			else
			{
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Group] The target has not been found.", l->GetLanguage()));
		return;
	}
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Group] You cannot call the target.", l->GetLanguage()));
		return;
	}
	...
	if (n == 0)
		l->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Group] You cannot call group members over to your current position.", l->GetLanguage()));
	else
	{
		int i = number(0, n - 1);
...
void CParty::Update()
{
	...
	for (it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		if (bLongTimeExpBonusChanged && ch->GetDesc())
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your group works well together, and group members near the group leader will receive an experience bonus.", ch->GetLanguage()));
