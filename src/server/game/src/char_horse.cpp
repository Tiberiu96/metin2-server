...
bool CHARACTER::StartRiding()
{
	if (IsDead() == true)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot mount a horse while knocked down.", GetLanguage()));
		return false;
	}
	
	if (IsPolymorphed())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot ride a Horse while you are transformed.", GetLanguage()));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot ride while you are wearing a Wedding Dress or a Tuxedo.", GetLanguage()));
		return false;
	}
	...
	{
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You do not have a Horse.", GetLanguage()));
		else if (GetHorseHealth() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your Horse is dead.", GetLanguage()));
		else if (GetHorseStamina() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your Horse's endurance is too low.", GetLanguage()));
...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pPetName)
{
	if ( bSummon )
	{
		...
		if (!m_chHorse)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Calling the Horse has failed.", GetLanguage()));
			return;
		}
