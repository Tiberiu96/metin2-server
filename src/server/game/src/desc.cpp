...
void DESC::Destroy()
{
	...
	if (!g_bAuthServer)
	{
		if (!m_bChangingChannel && m_accountTable.login[0] && m_accountTable.passwd[0])
		{
			TLogoutPacket pack;
...
void DESC::DisconnectOfSameLogin()
{
	if (GetCharacter())
	{
		...
		GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Someone has logged into your account. You will be disconnected from the server.", GetCharacter()->GetLanguage()));
		DelayedDisconnect(5);
	}
	else
	{
