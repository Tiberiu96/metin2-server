...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	if (pkSk->dwType == 0)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot train this skill up to Grand Master level.", GetLanguage()));
		return false;
	}
	...
	{
		if (GetSkillMasterType(dwSkillVnum) > SKILL_GRAND_MASTER)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You already are a Master of this skill. You cannot train this skill any further.", GetLanguage()));
		else
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your Skill is not high enough to become a Grand Master.", GetLanguage()));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_LANG("That did not work. Damn!", GetLanguage()));
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Training has failed. Please try again later.", GetLanguage()));
		LogManager::instance().CharLog(this, dwSkillVnum, "GM_READ_FAIL", "");
		return false;
	}

	ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_LANG("My body is full of energy!", GetLanguage()));
	ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_LANG("The training seems to be working already...", GetLanguage()));
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You successfully finished your training with the book.", GetLanguage()));
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	if (FN_should_check_exp(this))
	{
		...
		if ( GetExp() < need_exp )
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot read this due to your lack of experience.", GetLanguage()));
			return false;
		}
	}
	...
	if (pkSk->dwType != 0)
	{
		...
		{
			if (GetSkillMasterType(dwSkillVnum) > SKILL_MASTER)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot train this skill with a Book.", GetLanguage()));
			else
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This skill's level is not high enough to be trained with a Book.", GetLanguage()));
			return false;
		}
	}
			...
			if (pPC)
			{
				...
				if (number(1, 100) > percent)
				{
					...
					if (read_count >= need_bookcount)
					{
						...
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You have successfully finished your training with the Book.", GetLanguage()));
					...
					else
					{
						...
						{
							case 1:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_LANG("I'm making progress, but I still haven't understood everything.", GetLanguage()));
								break;
											
							case 2:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_LANG("These instructions are difficult to understand. I have to carry on studying.", GetLanguage()));
								break;

							case 3:
							default:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_LANG("I understand this chapter. But I've got to carry on working hard.", GetLanguage()));
								break;
						}

						ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You have to read %d more skill books to improve this skill.", GetLanguage()), need_bookcount - read_count);
						return true;
					}
				}
			}
...
EVENTFUNC(skill_gwihwan_event)
{
	...
	{
		//실패
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Teleportation has failed.", ch->GetLanguage()));
	}
	return 0;
}
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if (dwVnum == SKILL_HORSE_SUMMON)
	{
		...
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("No Horse here. Ask the Stable Boy.", GetLanguage()));
		else
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Please use an item to call a Horse.", GetLanguage()));
	...
	else
	{
		...
		if (test_server)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("%s FP-Consumption: %d", GetLanguage()), pkSk->szName, iNeededSP);
...
void CHARACTER::SkillLearnWaitMoreTimeMessage(DWORD ms)
{
	...
	//
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("I am burning inside, but it is calming down my body. My Chi has to stabilise.", GetLanguage()));
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("A little slow, but steady... Without stopping!", GetLanguage()));
	else if (ms < 10 * 60) // 10분
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("Yes, that feels great. My body is full of Chi.", GetLanguage()));
	else if (ms < 30 * 60) // 30분
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("I have read it! Now the Chi will spread through my body.", GetLanguage()));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("The training is completed.", GetLanguage()));
	}
	else if (ms < 1 * 3600) // 1시간
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("I am on the last page of the book. The training is nearly finished!", GetLanguage()));
	else if (ms < 2 * 3600) // 2시간
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("Nearly finished! Just a little bit more to go!", GetLanguage()));
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("Eureka! I have nearly finished reading it!", GetLanguage()));
	else if (ms < 6 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("Only a few more pages and then I'll have read everything.", GetLanguage()));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("I feel refreshed.", GetLanguage()));
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("Now I understand it!", GetLanguage()));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("Okay I have to stay concentrated!", GetLanguage()));
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("I keep reading the same line over and over again.", GetLanguage()));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("I do not want to learn any more.", GetLanguage()));
	}
	else //if (ms < 2 * 86400)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("It is a lot more complicated and more difficult to understand than I thought.", GetLanguage()));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_LANG("It's hard for me to concentrate. I should take a break.", GetLanguage()));
	}
	/*
	   str = "30%";
	   else if (ms < 3 * 86400)
