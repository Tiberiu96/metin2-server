	...
	int pc_give_item_from_special_item_group(lua_State* L)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (!item_gets[i])
			{
				if (dwVnums[i] == 1)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You have received %d Yang.", ch->GetLanguage()), dwCounts[i]);
				}
				else if (dwVnums[i] == 2)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("A mysterious light appears from the tree.", ch->GetLanguage()));
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You have received %d experience points.", ch->GetLanguage()), dwCounts[i]);
				}
			}
		}
		return 0;
...
teleport_area:
	...
	int pc_set_language(lua_State * L)
	{
		if (!lua_isstring(L, 1))
		{
			sys_err("pc_set_language: invalid argument");
			return 0;
		}
