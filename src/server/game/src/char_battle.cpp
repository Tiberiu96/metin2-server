...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You did not lose any Experience because of the Blessing of the Dragon God.", GetLanguage()));
		return;
	}
			...
			else
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked)
				{
					...
					if (number(1, 100) < iNoPenaltyProb)
						pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You did not drop any Item(s) as you are protected by the Dragon God.", pkKiller->GetLanguage()));
					else
					{
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[Guild] There are no experience points for hunting during a guild war.", pkKiller->GetLanguage()));
				}
			}
		}
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
		...
		if (pAttacker)
		{
			...
			if (iPenetratePct)
			{
				...
				if (number(1, 100) <= iPenetratePct)
				{
					...
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Additional Stabbing Weapon Damage %d", GetLanguage()), GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		if (type == DAMAGE_TYPE_NORMAL)
		{
			...
			if (GetPoint(POINT_BLOCK) && number(1, 100) <= GetPoint(POINT_BLOCK))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("%s Block! (%d%%)", pAttacker->GetLanguage()), GetName(), GetPoint(POINT_BLOCK));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("%s Block! (%d%%)", GetLanguage()), GetName(), GetPoint(POINT_BLOCK));
				}
		...
		else if (type == DAMAGE_TYPE_NORMAL_RANGE)
		{
			...
			if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("%s Dodge! (%d%%)", pAttacker->GetLanguage()), GetName(), GetPoint(POINT_DODGE));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("%s Dodge! (%d%%)", GetLanguage()), GetName(), GetPoint(POINT_DODGE));
				}
	...
	if (pAttacker)
	{
		...
		if (m_bDetailLog)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("%s[%d] attack position: %d %d", GetLanguage()), pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
		}
	}
