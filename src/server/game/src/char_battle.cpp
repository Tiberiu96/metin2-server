...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("용신의 가호로 경험치가 떨어지지 않았습니다."), GetLanguage()));
		return;
	}
			...
			else
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked)
				{
					...
					if (number(1, 100) < iNoPenaltyProb)
						pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("용신의 보호로 아이템이 떨어지지 않았습니다."), pkKiller->GetLanguage()));
					else
					{
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("<길드> 길드전중에는 사냥에 따른 이익이 없습니다."), pkKiller->GetLanguage()));
				}
			}
		}
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
		...
		if (pAttacker)
		{
			...
			if (iPenetratePct)
			{
				...
				if (number(1, 100) <= iPenetratePct)
				{
					...
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("관통 추가 데미지 %d"), GetLanguage()), GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		if (type == DAMAGE_TYPE_NORMAL)
		{
			...
			if (GetPoint(POINT_BLOCK) && number(1, 100) <= GetPoint(POINT_BLOCK))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s 블럭! (%d%%)"), pAttacker->GetLanguage()), GetName(), GetPoint(POINT_BLOCK));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s 블럭! (%d%%)"), GetLanguage()), GetName(), GetPoint(POINT_BLOCK));
				}
		...
		else if (type == DAMAGE_TYPE_NORMAL_RANGE)
		{
			...
			if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s 회피! (%d%%)"), pAttacker->GetLanguage()), GetName(), GetPoint(POINT_DODGE));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s 회피! (%d%%)"), GetLanguage()), GetName(), GetPoint(POINT_DODGE));
				}
	...
	if (pAttacker)
	{
		...
		if (m_bDetailLog)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG(LC_TEXT("%s[%d]가 공격 위치: %d %d"), GetLanguage()), pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
		}
	}
