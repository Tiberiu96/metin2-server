...
class CMobManager : public singleton<CMobManager>
{
	public:
		...
		bool			LoadMobNames();
		const char *	GetLocaleName(DWORD dwVnum, const char * lang);
	...
	private:
		...
		// per-language mob names: lang -> (vnum -> name)
		std::map<std::string, std::map<DWORD, std::string>> m_mapMobNamesByLang;
};
