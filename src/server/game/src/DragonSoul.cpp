...
bool DSManager::ExtractDragonHeart(LPCHARACTER ch, LPITEM pItem, LPITEM pExtractor)
{
	...
	if (pItem->IsEquipped())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The Dragon Stone cannot be removed.", ch->GetLanguage()));
		return false;
	}
	...
	if (fCharge < FLT_EPSILON)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Remaining duration extraction failed.", ch->GetLanguage()));
		return false;
	}
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Dragon Stone remaining duration has been extracted.", ch->GetLanguage()));
		return true;
	}
}
...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	if (!IsValidCellForThisItem(pItem, DestCell))
	{
		...
		if (iEmptyCell < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("There isn't enough space in your inventory.", ch->GetLanguage()));
			return false;
		}
		else
		{
	...
	// 캐릭터의 용혼석 추출 및 추가 혹은 제거. 부산물 제공.
	{
		...
		if (bSuccess)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Dragon Stone has been removed.", ch->GetLanguage()));
			pItem->AddToCharacter(ch, DestCell);
			return true;
		}
		else
		{
			...
			if (dwByProduct)
			{
				...
				if (pByProduct)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Removal of Dragon Stone failed. But you have received the following: %s", ch->GetLanguage()), pByProduct->GetName());
				else
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Removal of Dragon Stone failed.", ch->GetLanguage()));
			}
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Removal of Dragon Stone failed.", ch->GetLanguage()));
		}
...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	for (int i = 0; i < DRAGON_SOUL_REFINE_GRID_SIZE; i++)
	{
		...
		if (NULL != pItem)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This item is not required for improving the clarity level.", ch->GetLanguage()));
	...
	std::set <LPITEM>::iterator it = set_items.begin();
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This item cannot be advanced this way.", ch->GetLanguage()));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
	...
	if (ch->GetGold() < fee)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You do not have enough Yang to use this item.", ch->GetLanguage()));
	...
	if (result_grade > grade_idx)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Refinement up one class was successful.", ch->GetLanguage()));
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Refinement up one class failed.", ch->GetLanguage()));
...
bool DSManager::DoRefineStep(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	for (int i = 0; i < DRAGON_SOUL_REFINE_GRID_SIZE; i++)
	{
		...
		if (NULL != pItem)
		{
			...
			if (!pItem->IsDragonSoul())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You do not own the materials required to strengthen the Dragon Stone.", ch->GetLanguage()));
				SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
				return false;
			}
			set_items.insert(pItem);
	...
	std::set <LPITEM>::iterator it = set_items.begin(); 
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This item is not required for refinement.", ch->GetLanguage()));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	...
	if (result_step > step_idx)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Improvement of the clarity level successful.", ch->GetLanguage()));
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_SUCCEED, TItemPos (pResultItem->GetWindow(), pResultItem->GetCell()));
		return true;
	}
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Improvement of the clarity level failed.", ch->GetLanguage()));
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL, TItemPos (pResultItem->GetWindow(), pResultItem->GetCell()));
		return false;
	}
}
...
bool DSManager::DoRefineStrength(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (NULL != pDragonSoul)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("This Dragon Stone cannot be used for strengthening.", ch->GetLanguage()));
	...
	if (fnumber(0.f, 100.f) <= fProb)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Strengthening was successful.", ch->GetLanguage()));
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Strengthening failed.", ch->GetLanguage()));
		pDragonSoul->SetCount(pDragonSoul->GetCount() - 1);
		pRefineStone->SetCount(pRefineStone->GetCount() - 1);
		if (NULL != pResult)
			ch->AutoGiveItem(pResult, true);
