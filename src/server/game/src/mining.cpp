...
namespace mining
{
	...
	void CHEAT_MAX_PICK(LPCHARACTER ch, LPITEM item)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your mining points have reached their maximum. (%d)", ch->GetLanguage()), Pick_GetCurExp(pick));
	}
	...
	void PracticePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsPracticeSuccess(pick))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your mining points have reached their maximum level.", ch->GetLanguage()));
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You can get lumberjack Deokbae to upgrade your Pickaxe.", ch->GetLanguage()));
			}
			else
			{
				Pick_IncCurExp(pick);	

				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Your Mining Points have increased! (%d/%d)", ch->GetLanguage()),
	...
	EVENTFUNC(mining_event)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("You cannot mine without a Pick.", ch->GetLanguage()));
			return 0;
		}
		// END_OF_REFINE_PICK
		...
		if (!load)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Nothing to mine here.", ch->GetLanguage()));
			return 0;
		}
		...
		if (number(1, 100) <= iPct)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The mining has been successful.", ch->GetLanguage()));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The mining has failed.", ch->GetLanguage()));
		}
