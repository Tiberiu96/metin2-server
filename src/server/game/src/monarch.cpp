...
struct FHealMyEmpire 
{
	...
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("The energy is reflected because of the Emperors Blessing.", ch->GetLanguage()));
...
int CMonarch::HealMyEmpire(LPCHARACTER ch ,DWORD price)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("After %d seconds you can use the Emperors Blessing.", ch->GetLanguage()), ch->GetMCLTime(CHARACTER::MI_HEAL));
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("Not enough money in the treasury. Current: %u Required: %u", ch->GetLanguage()), GetMoney(Empire), price);
		return 0;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_LANG("[TEST_ONLY]Tax : %d", ch->GetLanguage()), GetMoney(Empire) - price);
	return 1;
}
