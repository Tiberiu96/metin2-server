	...
	if (pInfo != NULL)
	{
		switch (pInfo->state)
		{
			case 0:
				{
					++pInfo->state;
					BroadcastNotice(LC_TEXT("5 minutes remaining until the monsters attack!!!"));
				}
			...
			case 1:
				{
					++pInfo->state;
					BroadcastNotice(LC_TEXT("1 minute remaining until the monsters attack!!!"));
				}
			...
			case 2:
				{
					...
					BroadcastNotice(LC_TEXT("Monsters have started attacking the castle."));
			...
			case 3 :
				{
					...
					else
					{
						...
						{
							pInfo->state++;
							SendNoticeMap(LC_TEXT("Monsters show signs of retreating."), pInfo->nMapIndex, false);
						}
						else
						{
			...
			case 4 :
				{
					pInfo->state++;
					SendNoticeMap(LC_TEXT("Monsters have started retreating."), pInfo->nMapIndex, false);
					SendNoticeMap(LC_TEXT("Returning to the village shortly."), pInfo->nMapIndex, false);
			...
			case 6 :
				{
					...
					SendNoticeMap(LC_TEXT("The monster leader has appeared."), pInfo->nMapIndex, false);
					SendNoticeMap(LC_TEXT("Defeat Gwimokryongju within 30 minutes."), pInfo->nMapIndex, false);
			...
			case 7 :
				{
					if ( SECTREE_MANAGER::instance().GetMonsterCountInMap(pInfo->nMapIndex) <= 0 )
					{
						SendNoticeMap(LC_TEXT("Gwimokryongju and all his subordinates have been defeated."), pInfo->nMapIndex, false);
						SendNoticeMap(LC_TEXT("Returning to the village shortly."), pInfo->nMapIndex, false);
					...
					{
						SendNoticeMap(LC_TEXT("Gwimokryongju has retreated."), pInfo->nMapIndex, false);
						SendNoticeMap(LC_TEXT("Returning to the village shortly."), pInfo->nMapIndex, false);
...
bool CBattleArena::Start(int nEmpire)
{
	...
	char szBuf[1024];
	snprintf(szBuf, sizeof(szBuf), LC_TEXT("Monsters are marching towards %s's castle."), EMPIRE_NAME(m_nEmpire));
	BroadcastNotice(szBuf);
	BroadcastNotice(LC_TEXT("The castle will be attacked in 10 minutes."));
