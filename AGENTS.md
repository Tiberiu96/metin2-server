...
Nu modifica niciodata fisiere din alt proiect decat repo-ul curent. Pentru proiecte externe (ex: `metin2-client`), poti face doar analiza/read-only, cu exceptia cazului in care userul cere explicit editarea acelui proiect si cwd-ul este schimbat acolo.

## Raport final

La finalul fiecarei sarcini, specifica explicit ce foldere au fost modificate.
...
Vezi **`.Codex/references/commands_gm.md`**.
Spawn mob: `/mob <vnum>` (nu `/spawn`).

## Database Schema

Schema verificata: **`.Codex/references/db_schema.md`**
